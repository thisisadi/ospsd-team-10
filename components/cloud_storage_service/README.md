...
- Registers `s3_client_impl` with `cloud_storage_client_api` so `get_client()` resolves to S3.
- Exposes **OAuth 2.0 Authorization Code** endpoints (`/auth/login`, `/auth/callback`) using helpers from `s3_client_impl.oauth` and session storage from `s3_client_impl.token_store`.
...
```bash
uv run python -m cloud_storage_service
```
...
```bash
UVICORN_RELOAD=1 uv run python -m cloud_storage_service
```
...
| `HOST` / `PORT` | No | Bind address for `python -m cloud_storage_service`. |
