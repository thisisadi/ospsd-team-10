...
class S3CloudStorageClient:
    ...
    def __init__(self, s3: Any | None = None, config: S3Config | None = None) -> None:
    ...
    def upload(self, local_path: Path, remote_path: str) -> str:
        """Upload a file to S3."""
        self._s3.upload_file(str(local_path), self._config.bucket, remote_path)
        return remote_path

    def download(self, remote_path: str, local_path: Path) -> None:
        """Download a file from S3."""
        local_path.parent.mkdir(parents=True, exist_ok=True)
        self._s3.download_file(self._config.bucket, remote_path, str(local_path))
